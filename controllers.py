...
def index():
    posts = db(db.post).select(
        left=db.post.on(db.post.author == db.profile.user), orderby=~db.post.date_posted
    )
...
@unauthenticated("user/<user_id:int>", "user_posts.html")
